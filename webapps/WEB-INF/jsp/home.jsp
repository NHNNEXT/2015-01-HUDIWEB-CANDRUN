				...
				<c:forEach var="group" items="${friends}">
					<c:forEach var="friend" items="${group.value}">
						<c:if test="${group.key == 'requestesFriends' }">
							<li class="li-request-friend">
								<a href="javascript:void(0);" class="a-request-friend" id="${friend.email}">${friend.nickname}</a>
								<a href="javascript:void(0);" value="${friend.email}" class="btn-request-friend">수락</a>
							</li>
						</c:if>
						<c:if test="${group.key == 'availableFriends' }">
							<li class="li-available-friend">
								<a href="javascript:void(0);" class="a-available-friend" id="${friend.email}">${friend.nickname}</a>
							</li>
						</c:if>
						<c:if test="${group.key == 'nonavailableFriends' }">
							<li class="li-nonavailable-friend">
								<a href="javascript:void(0);" class="a-nonavailable-friend" id="${friend.email}">${friend.nickname}</a>
							</li>
						</c:if>
					</c:forEach>
				</c:forEach>
